...
public abstract partial class AbstractValidator<T> : IValidator<T>, IEnumerable<IValidationRule> {
	...
	/// </para>
	/// <para>
	/// If set to <see cref="FlexValidator.CascadeMode.Continue"/> then all rules in the class will execute regardless of failures.
	/// </para>
	/// <para>
	/// If set to <see cref="FlexValidator.CascadeMode.Stop"/> then execution of the validator will stop after any rule fails.
	/// </para>
	/// <para>
	...
	/// <see cref="DefaultValidatorOptions.Cascade{T, TProperty}(IRuleBuilderInitial{T, TProperty}, FlexValidator.CascadeMode)"/>.
